...
export default function AdminDashboard(props: Props) {
  ...
  async function loadUsers() {
    ...
    const users = await pizzaService.getUsers(userPage, 10, nameFilter);
    setUserList(users);
  }
    ...
    response = (
      <View title="Mama Ricci's kitchen">
                            ...
                            <tr className="border-neutral-500 border-t-2">
                              ...
                              <td className="px-6 py-1 whitespace-nowrap text-end text-sm font-medium">
                                <button
                                  ...
                                  data-testid={`close-franchise-${franchise.name}`}
                                >
                                  <TrashIcon />
                                  Close
                                </button>
        ...
        <div className="text-start py-8 px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-neutral-100 overflow-clip mb-4">
            <div className="flex flex-col">
              <div className="-m-1.5 overflow-x-auto">
                <div className="p-1.5 min-w-full inline-block align-middle">
                  <div className="overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {userList.users.map((user, index) => (
                          <tr key={index} className="bg-white">
                            <td className="px-6 py-2 text-sm text-gray-800 text-center">
                              {user.name}
                            </td>
                            <td className="px-6 py-2 text-sm text-gray-800 text-center">
                              {user.email}
                            ...
                            <td className="px-6 py-2 text-sm text-gray-800 text-center">
                              {(user.roles ?? [])?.[0]?.role ?? "Unknown Role"}
                            </td>
                            <td className="px-6 py-2 text-sm text-end">
                              <button
                                type="button"
                                className="px-2 py-1 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-1 border-orange-400 text-orange-400 hover:border-orange-800 hover:text-orange-800"
                      ...
                      <tfoot>
                        <tr>
                          ...
                          <td colSpan={2} className="text-end px-6 py-2">
                            <button
                              className="w-12 p-1 text-sm font-semibold rounded-lg border border-transparent bg-white text-grey border-grey m-1 hover:bg-orange-200 disabled:bg-neutral-300"
